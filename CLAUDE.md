...
用户确认 → Git提交
    ↓
需求变更(/change) → 重新进入开发流程 → 必须执行review和测试 → 同步更新文档
```
...
使用`/change`处理开发过程中的需求变更时，必须遵循以下流程：

1. **变更分析**：明确变更范围和影响
2. **代码实现**：按变更要求修改代码
3. **同步测试**：修改或新增对应的测试代码
4. **代码审查**：执行`/review`进行自检
5. **测试验证**：运行测试确保通过
6. **文档同步**：同步更新需求文档和技术方案文档

**重要**：`/change`流程不能跳过review和测试环节，必须确保代码质量和文档一致性。
...
| 阶段性确认 | 开发过程中遇到疑问及时确认 |
| 变更必测试 | 使用`/change`后必须同步修改测试代码并运行测试 |
| 变更必审查 | 使用`/change`后必须执行`/review`进行代码自检 |
| 变更必同步文档 | 使用`/change`后必须同步更新需求和技术方案文档 |
...
|-------|------|------|
| `/fix` | 修复Bug | 快速定位和修复 |
| `/change` | 需求变更 | 开发中需求调整，必须执行review和测试 |
| `/resume` | 恢复开发 | 中断后继续 |
| `/refactor` | 重构 | 代码优化 |
...
**需求变更**：
```
/change → 代码修改 → /gen-test → /review → 测试运行 → 文档同步
```
...
- 技术方案：`_技术`
- 任务文档：`_任务`
- SQL 脚本：无后缀，格式为 `{YYYYMMDD}_{中文模块名}.sql`

**示例**：
- 需求文档：`docs/req/20260117_建议反馈_需求.md`
- 技术方案：`docs/design/20260117_建议反馈_技术.md`
- 任务文档：`docs/task/20260117_建议反馈_任务.md`
- SQL 脚本：`docs/sql/20260117_建议反馈.sql`
