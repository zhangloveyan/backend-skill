...
| 规则 | 说明 | 示例 |
|------|------|------|
| 小写下划线 | snake_case 格式 | `user_id`, `created_at` |
...
```java
// 方法入口
log.info("开始处理xxx, param={}", param);

// 关键步骤
log.info("步骤1完成, result={}", result);

// 方法出口
log.info("处理完成, result={}", result);

// 异常记录（必须包含异常对象）
log.error("处理失败, param={}", param, e);
```
