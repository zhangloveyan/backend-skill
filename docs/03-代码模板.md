...
```java
package com.{company}.{project}.{module}.dto.request;
...
```java
package com.{company}.{project}.{module}.dto.response;
...
import com.{company}.{project}.{module}.dto.request.*;
import com.{company}.{project}.{module}.dto.response.{Entity}Response;
import com.{company}.{project}.{module}.entity.{Entity};
...
public class {Entity}ServiceImpl extends ServiceImpl<{Entity}Mapper, {Entity}>
    ...
    public Long create({Entity}CreateRequest request) {
        ...
        // 3. 保存
        this.save(entity);
        log.info("{description}创建成功, id={}", entity.getId());
    ...
    public void update({Entity}UpdateRequest request) {
        // 1. 检查是否存在
        {Entity} entity = this.getById(request.getId());
        if (entity == null) {
        ...
        log.info("{description}更新成功, id={}", entity.getId());
    }
    ...
    public void deleteById(Long id) {
        ...
        // 2. 删除（逻辑删除）
        this.removeById(id);
        log.info("{description}删除成功, id={}", id);
    }
...
@RestController
@RequestMapping("/api/web/v1/{module}")
@RequiredArgsConstructor
...
@RestController
@RequestMapping("/api/app/v1/{module}")
@RequiredArgsConstructor
