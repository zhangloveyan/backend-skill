...
| `open` | 开放接口 | 第三方对接、设备端 | `/toy/open/v1/oauth/token` |
| `callback` | 回调接口 | 第三方回调、Webhook | `/toy/callback/v1/payment/notify` |
| `internal` | 内部接口 | 服务内部调用 | `/toy/internal/v1/device/command` |
...
| 连字符分隔 | 多单词用连字符 | `/user-address` | `/userAddress` |
| 名词资源 | 资源用名词 | `/order` | `/get-order` |
| 动作用HTTP方法 | 不在URL写动词 | `POST /order` | `/order/create` |
| 无尾斜杠 | 末尾不加斜杠 | `/user` | `/user/` |
